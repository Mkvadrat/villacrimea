...
class ControllerCommonFooter extends Controller {
	public function index() {
		...
		$data['text_anketa_text'] = $this->language->get('text_anketa_text');
		$data['text_anketa_link'] = $this->language->get('text_anketa_link');
	...
	public function callMeView(){
		...
		if (isset($this->request->post['message'])) {$message = $this->request->post['message'];}
		if (isset($this->request->post['location'])) {$location = $this->request->post['location'];}
		...
		if (isset($name) && isset($tel)){
			...
			$mail->setHtml(html_entity_decode(sprintf('Имя: ' . $name . '<br>' . 'Телефон: ' . $tel . '<br>' . 'Email: ' . $email . '<br>' . 'Время записи: ' . $datetime . '<br>' . 'Сообщение: ' . $message . '<br>' . 'Ссылка на страницу: ' . $location), ENT_QUOTES, 'UTF-8'));
			$send = $mail->send();
