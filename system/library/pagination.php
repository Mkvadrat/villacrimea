...
class Pagination {
	...
	public function render() {
		if(preg_match( "#admin#", $_SERVER['REQUEST_URI'])){
			...
			$output = '<div class="paggination"><ul>';
	
			if ($page > 1) {
				if ($page - 1 === 1) {
					$output .= '<li><a href="' . str_replace(array('&amp;page={page}', '&page={page}'), '', $this->url) . '"></a></li>';
				} else {
					$output .= '<li><a href="' . str_replace('{page}', $page - 1, $this->url) . '"></a></li>';
				}
			}
			...
			if ($num_pages > 1) {
				...
				for ($i = $start; $i <= $end; $i++) {
					if ($page == $i) {
						$output .= '<li><span class="actual">' . $i . '</span></li>';
					} else {
						if ($i === 1) {
						$output .= '<li><a href="' . str_replace(array('&amp;page={page}', '&page={page}'), '', $this->url) . '">' . $i . '</a></li>';
						} else {
